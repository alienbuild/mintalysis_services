...
const processAssets = async (allTokens) => {
    for (const token of allTokens) {
        ...
        try {
            ...
            console.error("[ERROR] Failed to process token ID:", tokenIdNum, e);
        }
    }
...
const processTokenByType = async (
    tokenId,
    token,
    type,
    comicOrCollectibleId,
    ...
    if (token.token_id > 3880000) {
        ...
        created_mint_date = moment(token.metadata.mintDate).format(
            "YYYY-MM-DDTHH:mm:ss.SSS[Z]"
        );
    }
